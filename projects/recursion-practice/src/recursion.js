...
var sum = function(array) {
  //base
  if (array.length === 0) {
    //If no more array, return 0
    return 0;
  }
  if(array.length === 1) {
    //If one element left, return the last element to be added
    return array[0];
  }
  
  //recursion
  return array[0] + sum(array.slice(1));
  //Returning the first value of the array added to the rest of the array, doesn't sum until after the last function call
};
...
var isEven = function(n) {
  if (n === 0) {
    //If it starts as zero or subtracting 2 from n has resulted in 0
    return true;
  } else if (n === 1) {
    //It must be odd if we've subtracted 2 continuosly, or it may be entered as 1 immediately
    return false;
  } else if (n > 1) {
    //Keep subtracting 2 if it's greater than 1 because we don't know if it's even or odd yet
    return isEven(n - 2);
  } else {
    //It's a negative number and we need to change it
    return isEven(n * - 1);
  }
...
var palindrome = function(string) {
  ...
  //Gets rid of capital letters to compare letters as lower case

  string = string.replace(" ", "");
  //Gets rid of all of the spaces in the string

  //base
  if (string.length === 1) {
    return true;
  }
  //If there's only one letter either one letter was entered or the function is done
  //analyzing the first and last letters, meaning it is a palindrome
  
  if (string[0] !== string[string.length - 1]) {
    //If the first and last letters aren't equal, then it isn't a palindrome
    return false;
  }

  //recursion
  //During recursion, returning the string but removing the first and last letter so that the
  //next ones can be compared
...
var letterTally = function(str, obj = {}) {
  ...
  if (obj.hasOwnProperty(str[str.length - 1])) {
    obj[str[str.length - 1]] += 1;
  } else {
    obj[str[str.length - 1]] = 1;
  }
  return letterTally(str.substring(0, str.length - 1), obj);
};
